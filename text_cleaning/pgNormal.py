...
oldpklFilePath = 'courtLB.pkl' # textobj[0] for label, textobj[1] for text
newpklFilePath = 'court-LbParag.pkl' # textobj['label'] for label, textobj['text'] for text
...
def paragNormalize(textObj):
    ...
    '''
    paragList = []
    commonLabel = isInformative(textObj[0])
    ...
    if commonLabel:
        ...
        for i, line in enumerate(raw_text):
            ...
            if i == len(raw_text) - 1:
                workingParag += sents
                paragList.append({'label': commonLabel, 'text': workingParag})
                workingParag = []
            else:
                lastSent = sents[-1]
